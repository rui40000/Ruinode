...
| 素材 | 结果 |
|:-----|:-----|
| 光效、火焰、烟雾、粒子、UI 特效 | ✅ 最佳选择，半透明层次完整保留 |
| 绿幕/品红等**与素材反差大**的底色 | ✅ 实体素材也能扣干净 |
| **黑底 + 暗色实体** | ❌ **暗部会被当成背景扣掉** |

实测同一个实体素材（alpha 真值恒为 1）：

- 黑底：亮部还原 alpha 0.900，**暗部只有 0.080** —— 黑头发、深色衣服、鞋子会被扣穿
- 绿幕：亮部 0.900，**暗部 0.940** ✓ 正常

原因是黑底 unmult 本质在用"亮度当不透明度"，这对发光物成立，对实体不成立。**所以黑底的角色素材请用 [Lucida](#23-lucida-抠图--lucida-matting) 或 [FeyNobg](#22-feynobg-抠图--feynobg-matting) 这类语义抠图**，unmult 留给光效。
